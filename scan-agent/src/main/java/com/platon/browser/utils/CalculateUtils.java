...
public class CalculateUtils {
	 ...
	 * 轮换利润
	 * @param profits 利润列表
	 * @param cumulativeProfit 累计【质押/委托】利润
	 * @param curSettleEpoch 当前结算周期轮数
	 * @param chainConfig 链配置
	 */
	public static void rotateProfit(List<PeriodValueElement> profits,BigDecimal cumulativeProfit,BigInteger curSettleEpoch,BlockChainConfig chainConfig)  {
		// 添加上一周期的收益
		if(curSettleEpoch.longValue()==0) cumulativeProfit=BigDecimal.ZERO; // 如果当前结算周期是0则利润为0
		PeriodValueElement pve = new PeriodValueElement();
		pve.setPeriod(curSettleEpoch.longValue());
		pve.setValue(cumulativeProfit);
		profits.add(pve);
	...
	public static BigDecimal calculateAnnualizedRate(
		...
		if(profits.size()==1){
			// 只有1条年化率计算信息，直接取出来计算
			profitSum = profits.get(0).getValue();
			costSum = costs.get(0).getValue();
		}
		...
		if(profits.size()>1){
			...
			profits.sort((c1, c2) -> Integer.compare(0, c1.getPeriod().compareTo(c2.getPeriod())));
			// 最旧利润累计
			PeriodValueElement oldest = profits.get(profits.size()-1);
			// 最新利润累计
			PeriodValueElement newest = profits.get(0);
			// 利润=最新利润累计-最旧利润累计
			profitSum=newest.getValue().subtract(oldest.getValue()).abs();
